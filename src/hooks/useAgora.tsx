...
export default function useAgora(client: IAgoraRTCClient | undefined): {
  ...
  async function join(appid: string, channel: string, token?: string, uid?: string | number | null) {
    ...
    const joinClient = await client.join(appid, channel, token || null, uid);
    console.log(joinClient);
  ...
  async function leave() {
    console.log({ 'LOCAL VIDEO': localVideoTrack });
    console.log({ 'LOCAL AUDIO': localAudioTrack });
    if (localAudioTrack) {
  ...
  useEffect(() => {
    ...
    console.log({ CLIENTCHANGE: client });
